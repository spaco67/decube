/*
  # Orders Schema

  1. New Tables
    - `orders`
      - `id` (uuid, primary key)
      - `table_id` (uuid, foreign key to tables)
      - `waiter_id` (uuid, foreign key to users)
      - `status` (text)
      - `total_amount` (numeric)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)
      - `completed_at` (timestamp)
    
    - `order_items`
      - `id` (uuid, primary key)
      - `order_id` (uuid, foreign key to orders)
      - `menu_item_id` (uuid, foreign key to menu_items)
      - `quantity` (integer)
      - `price` (numeric)
      - `status` (text)
      - `notes` (text)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on both tables
    - Add policies for authenticated users
*/

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Staff can view relevant orders" ON orders;
DROP POLICY IF EXISTS "Waiters can create orders" ON orders;
DROP POLICY IF EXISTS "Staff can update relevant orders" ON orders;
DROP POLICY IF EXISTS "Staff can view relevant order items" ON order_items;
DROP POLICY IF EXISTS "Waiters can create order items" ON order_items;
DROP POLICY IF EXISTS "Staff can update relevant order items" ON order_items;

-- Create orders table
DO $$ BEGIN
  CREATE TABLE IF NOT EXISTS orders (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    table_id uuid REFERENCES tables(id),
    waiter_id uuid REFERENCES users(id),
    status text NOT NULL DEFAULT 'pending',
    total_amount numeric(10,2) NOT NULL DEFAULT 0,
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now(),
    completed_at timestamptz,
    CONSTRAINT orders_status_check CHECK (status IN ('pending', 'in-progress', 'ready', 'completed', 'cancelled'))
  );
EXCEPTION
  WHEN duplicate_table THEN NULL;
END $$;

-- Create order_items table
DO $$ BEGIN
  CREATE TABLE IF NOT EXISTS order_items (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    order_id uuid REFERENCES orders(id),
    menu_item_id uuid REFERENCES menu_items(id),
    quantity integer NOT NULL DEFAULT 1,
    price numeric(10,2) NOT NULL,
    status text NOT NULL DEFAULT 'pending',
    notes text,
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now(),
    CONSTRAINT order_items_status_check CHECK (status IN ('pending', 'in-progress', 'ready', 'completed', 'cancelled'))
  );
EXCEPTION
  WHEN duplicate_table THEN NULL;
END $$;

-- Enable RLS
ALTER TABLE orders ENABLE ROW LEVEL SECURITY;
ALTER TABLE order_items ENABLE ROW LEVEL SECURITY;

-- Policies for orders
CREATE POLICY "Staff can view relevant orders"
  ON orders
  FOR SELECT
  TO authenticated
  USING (
    auth.uid() = waiter_id OR
    EXISTS (
      SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'
    ) OR
    EXISTS (
      SELECT 1 FROM order_items oi
      JOIN menu_items mi ON oi.menu_item_id = mi.id
      WHERE oi.order_id = orders.id AND (
        (mi.preparation_type = 'bar' AND EXISTS (
          SELECT 1 FROM users WHERE id = auth.uid() AND role = 'barman'
        )) OR
        (mi.preparation_type = 'kitchen' AND EXISTS (
          SELECT 1 FROM users WHERE id = auth.uid() AND role = 'kitchen'
        ))
      )
    )
  );

CREATE POLICY "Waiters can create orders"
  ON orders
  FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM users
      WHERE id = auth.uid() AND (role = 'waiter' OR role = 'admin')
    )
  );

CREATE POLICY "Staff can update relevant orders"
  ON orders
  FOR UPDATE
  TO authenticated
  USING (
    auth.uid() = waiter_id OR
    EXISTS (
      SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'
    )
  );

-- Policies for order_items
CREATE POLICY "Staff can view relevant order items"
  ON order_items
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM orders o
      JOIN users u ON u.id = auth.uid()
      WHERE o.id = order_items.order_id AND (
        u.role = 'admin' OR
        o.waiter_id = u.id OR
        (u.role = 'barman' AND EXISTS (
          SELECT 1 FROM menu_items mi
          WHERE mi.id = order_items.menu_item_id AND mi.preparation_type = 'bar'
        )) OR
        (u.role = 'kitchen' AND EXISTS (
          SELECT 1 FROM menu_items mi
          WHERE mi.id = order_items.menu_item_id AND mi.preparation_type = 'kitchen'
        ))
      )
    )
  );

CREATE POLICY "Waiters can create order items"
  ON order_items
  FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM users u
      JOIN orders o ON o.id = order_items.order_id
      WHERE u.id = auth.uid() AND (u.role = 'waiter' OR u.role = 'admin')
    )
  );

CREATE POLICY "Staff can update relevant order items"
  ON order_items
  FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users u
      JOIN orders o ON o.id = order_items.order_id
      WHERE u.id = auth.uid() AND (
        u.role = 'admin' OR
        o.waiter_id = u.id OR
        (u.role = 'barman' AND EXISTS (
          SELECT 1 FROM menu_items mi
          WHERE mi.id = order_items.menu_item_id AND mi.preparation_type = 'bar'
        )) OR
        (u.role = 'kitchen' AND EXISTS (
          SELECT 1 FROM menu_items mi
          WHERE mi.id = order_items.menu_item_id AND mi.preparation_type = 'kitchen'
        ))
      )
    )
  );